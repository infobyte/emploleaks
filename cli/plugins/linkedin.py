...
class LinkedinModule:
    ...
    def do_show(self, args):
        ...
        for opt in self.options:
            if hide_pass and opt['name'] == 'JSESSIONID':
                table.append([opt['name'], '*' * len(self.options[1]['value']), 'yes' if opt['required'] else 'no',
                              opt['description']])
                continue
    ...
    def get_company_info(self, name):
        ...
        if response.status_code != 200:
            print(
                f"[{Fore.RED}-{Style.RESET_ALL}] Unexpected HTTP repsonse code {response.status_code} when trying to get the company info:")
            return
        ...
        try:
            ...
            for _ in outer_loops:
                for page in range(depth):
                    ...
                    new_names = 0 + len(found_profiles)
    ...
    def find_employees(self, result):
        ...
        for body in result_json['elements']:
            ...
            try:
                ...
                print(f"profile has no occupation: {profile}")
